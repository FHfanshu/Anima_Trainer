...
def _sanitize_config_name(config_name: str) -> str:
    ...
    if safe_name != config_name:
        raise HTTPException(status_code=400, detail="配置名非法")
...
async def load_config(config_name: str):
    ...
    try:
        ...
        config_file = PROJECT_ROOT / "config" / "autosave" / f"{safe_name}.yaml"
        
        if not config_file.exists():
            raise HTTPException(status_code=404, detail=f"配置文件 '{safe_name}' 不存在")
