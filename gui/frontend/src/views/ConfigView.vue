    ...
    >
      <el-tabs type="border-card" v-model="activeTab">
        ...
        <el-tab-pane label="模型设置" name="model">
          <el-row :gutter="20">
            <el-col :xs="24" :md="12">
              <el-form-item label="Transformer 权重路径">
                <el-input
                  v-model="configStore.config.pretrained_model_name_or_path"
                  placeholder="选择 transformer .safetensors"
                  clearable
                >
                  <template #append>
          ...
          <el-row :gutter="20">
            <el-col :xs="24" :md="12">
              <el-form-item label="Qwen 模型目录">
                <el-input
                  v-model="configStore.config.qwen_path"
                  placeholder="选择 text_encoders 目录"
                  clearable
                >
                  <template #append>
                    <el-button @click="browseQwen">浏览</el-button>
                  </template>
                </el-input>
              </el-form-item>
            </el-col>
...
const browseQwen = async () => {
  try {
    const resp = await api.system.pickFolder()
    const path = resp.data?.path
    if (path) configStore.config.qwen_path = path
  } catch {
    ElMessage.error('打开 Qwen 模型目录选择对话框失败')
  }
}
